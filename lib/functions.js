          ...
          for (let i=0; i < rows.length; i++) {
            if (deptName === rows[i].dept_name) {
              deptId.push(rows[i].dept_id)   // Department id that will be used for sql update
                ...
                .then ( ([rows, fields]) => {
                  log(chalk.cyan("==========================================================================================================="));
                  log(`!----- Total budget for ${deptName} department -----!`);
                  log("");
                  console.table(rows);
                  log(chalk.cyan("==========================================================================================================="));  
                })
      ...
      inquirer.prompt ([
        {
          type: 'list',
          name: 'newRoleDept',
          message: `Which department does the ${answers.newRole} belong?`,
          choices: departments //shows each department from departments table
        }
      ])
      .then((answer) => {
        ...
        inquirer.prompt ([
          {
            type: 'list',
            name: 'newEmpRole',
            message: `What is ${answers.newEmpFirst} ${answers.newEmpLast}'s role?`,
                      ...
                      inquirer.prompt ([
                        {
                          type: 'list',
                          name: 'newEmpManager',
                          message: `Who is ${answers.newEmpFirst} ${answers.newEmpLast}'s manager?`,
